...
QVariant TaskListViewModel::data(const QModelIndex &index, int role) const {
  ...
  case DueAtRole:
    return task.dueAt.isValid() ? task.dueAt.toString(Qt::ISODate) : QVariant();
  case IsPinnedRole:
    return task.isPinned;
  case SectionRole:
    return getTaskSection(task);
  case TagsRole:
    return task.tags;
  case ListRole:
    return normalizedListName(task.listId);
  default:
    return QVariant();
  }
}
...
void TaskListViewModel::refreshNotesListCache() {
  m_notesListNames.clear();
  const QStringList all = m_taskService->getLists();
  for (const QString &name : all) {
    if (m_taskService->getListType(name) == QLatin1String("Notes List"))
      m_notesListNames.insert(name);
  }
}
...
void TaskListViewModel::loadTasks() {
  ...
  for (const Task &task : allTasks) {
    ...
    if (!m_filterTag.isEmpty() &&
        !task.tags.contains(m_filterTag, Qt::CaseInsensitive)) {
      continue;
    }

    if (!m_filterList.isEmpty() &&
        normalizedListName(task.listId)
                .compare(m_filterList, Qt::CaseInsensitive) != 0) {
      continue;
    }

    // Exclude Notes List tasks unless explicitly viewing that Notes List
    {
      const QString normalizedId = normalizedListName(task.listId);
      const bool isNotesTask = m_notesListNames.contains(normalizedId);
      const bool viewingNotesList =
          !m_filterList.isEmpty() &&
          m_notesListNames.contains(normalizedListName(m_filterList));

      if (isNotesTask && !viewingNotesList)
        continue;
    }
    ...
    if (!m_filterDate.isEmpty() && m_filterDate != "Inbox" &&
        m_filterDate != "All" && m_filterDate != "Completed" &&
        m_filterDate != "Calendar") {
      if (m_filterDate == "Today") {
        if (!task.dueAt.isValid() || task.dueAt.date() != today)
          continue;
      } else if (m_filterDate == "Next 7 Days") {
        if (!task.dueAt.isValid() || task.dueAt.date() < today ||
            task.dueAt.date() > today.addDays(7))
          continue;
...
void TaskListViewModel::addTask(const QString &title,
                                const QString &description, int priority,
                                const QString &dueAt, const QStringList &tags,
                                const QString &listName) {
  QString targetList =
      listName.trimmed().isEmpty()
          ? (m_filterList.isEmpty() ? QStringLiteral("Inbox") : m_filterList)
          : listName;
...
void TaskListViewModel::toggleTaskCompletion(int row) {
  ...
  qDebug() << "[TaskListViewModel] Task found:" << task.title
           << "ID:" << task.id;
...
void TaskListViewModel::softDeleteTask(int row) {
  ...
  if (row < 0 || row >= m_tasks.count()) {
    qDebug() << "[TaskListViewModel] Invalid row for delete! Count is:"
             << m_tasks.count();
    return;
  }
  qDebug() << "[TaskListViewModel] Deleting task:" << m_tasks[row].title
           << "ID:" << m_tasks[row].id;
  m_taskService->deleteTask(m_tasks[row].id);
}

void TaskListViewModel::moveTask(int fromRow, int toRow) {
  if (fromRow < 0 || fromRow >= m_tasks.count() || toRow < 0 ||
      toRow >= m_tasks.count() || fromRow == toRow)
    return;

  // We update the orderIndex to shift them.
  // However, since m_tasks is sorted by groups, moving across groups might
  // break sorting locally unless we also change the group properties. For
  // "local only", we'll just swap orderIndex for now.
...
QStringList
TaskListViewModel::getListsForFolder(const QString &folderName) const {
...
void TaskListViewModel::createList(const QString &listName,
                                   const QString &color,
...
void TaskListViewModel::createTag(const QString &tagName, const QString &color,
...
QString TaskListViewModel::selectedTaskDueAt() const {
  if (m_selectedTaskIndex >= 0 && m_selectedTaskIndex < m_tasks.size()) {
    ...
    if (dt.isValid())
      return dt.toString(Qt::ISODate).left(10);
  }
  return "";
}
...
void TaskListViewModel::updateSelectedTaskDescription(
...
int TaskListViewModel::getTodayCount() const {
  ...
  int count = 0;
  for (const Task &task : allTasks) {
    if (m_notesListNames.contains(normalizedListName(task.listId)))
      continue;
...
int TaskListViewModel::getTagTaskCount(const QString &tag) const {
  ...
  for (const Task &task : allTasks) {
    if (m_notesListNames.contains(normalizedListName(task.listId)))
      continue;
    if (!task.isCompleted && task.status != "trashed" &&
        task.tags.contains(tag, Qt::CaseInsensitive)) {
      count++;
    }
  }
  return count;
